<template>
	<view class="tutor-list-container">
		...
		>
			...
			<!-- 搜索和筛选区域（scroll-view 内勿用 fixed，支付宝上易层级错乱；用 sticky） -->
			<view class="filter-section">
				...
				<view class="search-bar">
					<view class="search-input-box" @click="showSearchDialog = true">
						...
						<text class="search-bar-placeholder-text">{{ searchKeyword || '搜索科目、年级、城市...' }}</text>
					</view>
...
export default {
	...
	data() {
		return {
			...
			// 滚动相关（供 scroll-view 绑定，需程序化改滚动位置时可赋值）
			scrollTop: 0,
		...
		async loadBannerList() {
			try {
				...
				if (isSuccess && data.length > 0) {
					// 只显示状态为启用(1)的横幅（接口可能返回 "1"/"0" 字符串）
					let banners = data.filter(banner => Number(banner && banner.status) === 1)
					
					// 处理图片URL，如果是相对路径则拼接完整URL
					banners = banners.map(banner => {
						// 统一字段类型，避免后续判断/排序异常
						if (banner && banner.sort_order !== undefined && banner.sort_order !== null) {
							banner.sort_order = Number(banner.sort_order)
						}
...
/* 搜索和筛选区域：sticky 在 scroll-view 内吸顶，避免支付宝上 fixed 穿透/叠层错误 */
.filter-section {
	position: sticky;
	top: 0;
	left: 0;
