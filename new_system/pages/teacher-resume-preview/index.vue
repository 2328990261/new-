...
export default {
	...
	onLoad(options) {
		...
		// 如果有 teacher_id 且不是从注册页面来的，从后端加载教师数据
		...
		// 兼容旧的URL参数传递方式
			...
			try {
				...
				this.resumeData = { ...this.resumeData, ...data }
				console.log('加载的简历数据:', this.resumeData)
			} catch (e) {
	...
	methods: {
		...
		goToEdit() {
			...
			// 有teacherId，跳转到编辑模式第一步
			uni.navigateTo({
				url: `/pages/teacher-register/index?mode=edit&teacher_id=${this.teacherId}&step=1`
			})
		},
