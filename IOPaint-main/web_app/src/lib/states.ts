...
type AppAction = {
  ...
  setInputFiles: (files: File[], startIndex?: number) => Promise<void>
  gotoInputFile: (index: number) => Promise<void>
...
export const useStore = createWithEqualityFn<AppState & AppAction>()(
  persist(
    immer((set, get) => ({
      ...
      setInputFiles: async (files: File[], startIndex: number = 0) => {
        const safeFiles = files.filter((f) => f.type?.startsWith("image/"))
        ...
        set((state) => {
          state.inputFiles = safeFiles
          state.inputFileIndex = safeIndex
        })
        await get().setFile(safeFiles[safeIndex])
      },

      gotoInputFile: async (index: number) => {
        const list = get().inputFiles
        ...
        await get().setFile(list[safeIndex])
      },
