    ...
    >
      <template #header>
        <div style="display: flex; justify-content: space-between; align-items: center; width: 100%;">
          <span style="font-size: 18px; font-weight: 600;">投递详情</span>
          <el-button 
            type="primary" 
            link 
            @click="openInNewTab"
      ...
      <div v-if="currentApplication && teacherResume" style="max-height: 70vh; overflow-y: auto;">
        <!-- 投递状态和时间 -->
        <el-alert 
        ...
        <el-descriptions v-if="showResumeContact" :column="2" border style="margin-bottom: 24px;">
...
import { Search, RefreshLeft, Lock, Location, Plus, Close, List, Clock, CircleCheck, CircleClose, FullScreen, DocumentCopy, View } from '@element-plus/icons-vue'
import { 
  getApplicationList, 
...
const handleCustomDateChange = (dates) => {
  if (dates && dates.length === 2) {
    searchForm.startTime = dates[0] + ' 00:00:00'
    searchForm.endTime = dates[1] + ' 23:59:59'
    handleSearch()
  } else {
...
const calculateDateRange = (range) => {
  ...
  switch (range) {
    case 'today':
      startDate = endDate = formatLocalDate(now)
      break
    case 'yesterday':
...
const copyResume = () => {
  ...
  if (teacherResume.value.advantage_tags && teacherResume.value.advantage_tags.length > 0) {
    resumeText += `\n优势标签：${teacherResume.value.advantage_tags.join('、')}\n`
  }
  ...
  if (teacherResume.value.experience && teacherResume.value.experience.length > 0) {
    ...
    teacherResume.value.experience.forEach((exp, index) => {
      resumeText += `${index + 1}. ${exp.subject || ''}`
      if (exp.start_date && exp.end_date) {
        resumeText += ` (${exp.start_date} - ${exp.end_date})`
      }
      if (exp.location) {
...
const openInNewTab = () => {
  if (!currentApplication.value) {
    ElMessage.error('无法获取投递信息')
    return
  }
  
  // 保存当前tab和搜索条件到localStorage
  localStorage.setItem('application_current_tab', activeTab.value)
  localStorage.setItem('application_search_form', JSON.stringify({
    teacher_name: searchForm.teacher_name,
    tutor_title: searchForm.tutor_title,
  ...
  // 构建详情页路径
  const detailPath = `/applications/${currentApplication.value.id}`
  const title = `投递详情 #${currentApplication.value.id}`
  
  // 添加到标签页
  tabsStore.addTab(detailPath, title, true)
