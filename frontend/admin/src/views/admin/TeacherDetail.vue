      ...
      <div class="tab-content">
        ...
        <div v-show="activeTab === 'certification'">
          ...
          <el-form label-width="120px">
            ...
            <el-form-item label="审核时间" v-if="teacher.review_time">
              <span>{{ teacher.review_time }}</span>
            </el-form-item>
            
            <el-form-item label="审核备注" v-if="teacher.review_note">
              <span style="color: #909399;">{{ teacher.review_note }}</span>
            </el-form-item>
            
            <el-form-item label="认证审核">
              <div style="display: flex; flex-direction: column; gap: 12px;">
                <div style="display: flex; align-items: center; gap: 10px;">
                  <span style="width: 100px;">实名认证：</span>
                  <el-switch 
                    v-model="certificationForm.real_name_verified" 
                    :active-value="1" 
                    :inactive-value="0"
                    active-text="已认证" 
                    inactive-text="未认证"
                ...
                <div style="display: flex; align-items: center; gap: 10px;">
                  <span style="width: 100px;">学历认证：</span>
                  <el-switch 
                    v-model="certificationForm.education_verified" 
                    :active-value="1" 
                    :inactive-value="0"
                    active-text="已认证" 
                    inactive-text="未认证"
                ...
                <div style="display: flex; align-items: center; gap: 10px;">
                  <span style="width: 100px;">教师认证：</span>
                  <el-switch 
                    v-model="certificationForm.teacher_verified" 
                    :active-value="1" 
                    :inactive-value="0"
                    active-text="已认证" 
                    inactive-text="未认证"
            ...
            <el-form-item label="审核操作">
              <div style="display: flex; flex-direction: column; gap: 12px; width: 100%;">
                <el-radio-group v-model="certificationForm.review_status">
                ...
                <el-input
                  v-model="certificationForm.review_note"
                  type="textarea"
                  :rows="3"
                  placeholder="请输入审核备注（选填，拒绝时建议填写原因）"
                  maxlength="200"
                  show-word-limit
                  style="width: 100%;"
                />
                ...
                <div style="display: flex; gap: 10px;">
                  <el-button type="primary" @click="submitCertificationReview" :loading="reviewLoading">
                    提交审核
                  </el-button>
                  <el-button @click="resetCertificationForm">
                    重置
                  </el-button>
                </div>
              </div>
  ...
  try {
    ...
    if (res.success) {
      ...
      certificationForm.value = {
        real_name_verified: res.data.real_name_verified || 0,
        education_verified: res.data.education_verified || 0,
        teacher_verified: res.data.teacher_verified || 0,
        review_status: res.data.review_status || 'pending',
...
const generateResumePoster = async () => {
  return new Promise((resolve, reject) => {
    ...
    let calculatedY = cardY + 70 // 起始位置
    
    // 标题区域
    calculatedY += 70 // 标题 + 下划线 + 间距
    
    // 头像和基本信息区域
    calculatedY += 150 // 头像和信息区域的高度
    ...
    if (teacher.value.school || teacher.value.major) {
      calculatedY += 60 // 标题
      if (teacher.value.school) calculatedY += 50
      if (teacher.value.major) calculatedY += 50
      calculatedY += 20 // 底部间距
    }
    ...
    if (hasTeachingInfo) {
      calculatedY += 60 // 标题
      if (teacher.value.hourly_rate) calculatedY += 55
      if (teacher.value.subject_names && teacher.value.subject_names.length > 0) calculatedY += 55
      if (teacher.value.district_names && teacher.value.district_names.length > 0) {
        const districtText = teacher.value.district_names.join('、')
        const lines = Math.ceil(districtText.length * 22 / (contentWidth - 105))
        calculatedY += lines * 45
      }
      calculatedY += 20 // 底部间距
    }
    
    // 自我介绍
    if (teacher.value.self_intro && teacher.value.self_intro.trim()) {
      calculatedY += 60 // 标题
      const introLines = Math.min(5, Math.ceil(teacher.value.self_intro.length * 21 / (contentWidth - 15)))
      calculatedY += introLines * 45
      calculatedY += 20 // 底部间距
    }
    
    // 个人优势
    if ((teacher.value.personal_advantage && teacher.value.personal_advantage.trim()) || 
        (teacher.value.advantage_tags && teacher.value.advantage_tags.length > 0)) {
      calculatedY += 60 // 标题
      if (teacher.value.personal_advantage && teacher.value.personal_advantage.trim()) {
        const advantageLines = Math.min(4, Math.ceil(teacher.value.personal_advantage.length * 21 / (contentWidth - 15)))
        calculatedY += advantageLines * 45 + 30
      }
      if (teacher.value.advantage_tags && teacher.value.advantage_tags.length > 0) {
        const tagLines = Math.ceil(Math.min(6, teacher.value.advantage_tags.length) / 3) // 估算标签行数
        calculatedY += tagLines * 45 + 20
      } else {
        calculatedY += 20 // 底部间距
      }
    }
    ...
    // 教学经历
    if (teacher.value.experience && teacher.value.experience.length > 0) {
      calculatedY += 60 // 标题
      const experiencesToShow = teacher.value.experience.slice(0, 3)
      experiencesToShow.forEach((exp, index) => {
        calculatedY += 45 // 标题
        if (exp.start_date || exp.end_date) calculatedY += 38
        if (exp.location) calculatedY += 38
        if (exp.description) {
          const descLines = Math.min(3, Math.ceil(exp.description.length * 20 / (contentWidth - 45)))
          calculatedY += descLines * 38
        } else {
          calculatedY -= 10
        }
        if (index < experiencesToShow.length - 1) calculatedY += 25
      })
      calculatedY += 10 // 底部间距
    }
    
    calculatedY += 10 // 底部留白（从15减少到10）
    const contentEndY = calculatedY
    const finalHeight = Math.max(contentEndY + 40, 800) // 加上底部文字空间（从45减少到40），最小800px
    
    // 第二阶段：使用计算出的高度创建画布并绘制
    const canvas = document.createElement('canvas')
    const ctx = canvas.getContext('2d')
    canvas.width = width
    ...
    // === 背景设计 ===
    const bgGradient = ctx.createLinearGradient(0, 0, 0, finalHeight)
    ...
    ctx.fill()
    ctx.beginPath()
    ctx.arc(100, finalHeight - 100, 150, 0, Math.PI * 2)
    ctx.fill()
    ctx.globalAlpha = 1
    ...
    // 白色内容卡片
    const cardHeight = contentEndY - cardY + 20
    ctx.fillStyle = '#ffffff'
    ...
    // 在开发环境和生产环境中，都通过后端代理接口获取图片base64
    // 这样可以避免跨域问题
    if (teacher.value.avatar) {
      console.log('通过后端代理加载头像:', teacher.value.avatar)
      loadAvatarViaProxy()
    } else {
    ...
    // 通过后端代理加载头像
    function loadAvatarViaProxy() {
      // 构建完整的图片URL
      let imageUrl = teacher.value.avatar
      ...
      console.log('请求代理接口，图片URL:', imageUrl)
      
      // 调用后端代理接口
      const proxyUrl = `/admin/api/image-proxy?url=${encodeURIComponent(imageUrl)}`
      fetch(proxyUrl, {
        method: 'GET',
        headers: {
        ...
        .then(response => {
          console.log('代理接口响应状态:', response.status)
          return response.json()
        })
        .then(result => {
          console.log('代理接口返回结果:', result)
          if (result.code === 200 && result.data && result.data.base64) {
            // 使用base64数据加载图片
            const img = new Image()
            img.onload = function() {
            ...
            img.onerror = function() {
              console.error('base64图片加载失败，使用默认头像')
              drawDefaultAvatar()
              continueDrawing()
            }
            img.src = result.data.base64
          } else {
            console.error('代理接口返回错误:', result.msg || '未知错误')
            drawDefaultAvatar()
            continueDrawing()
          }
        })
        .catch(error => {
          console.error('通过代理加载头像失败:', error)
          drawDefaultAvatar()
          continueDrawing()
        })
    }
    ...
    function drawDefaultAvatar() {
      ...
      ctx.textAlign = 'center'
      ctx.fillText(teacher.value.name?.charAt(0) || '?', avatarX, avatarY + 15)
    ...
    function continueDrawing() {
      // 右侧信息起始位置
      const infoX = padding + 200
      let infoY = y + 15
      
      // === 第一排：姓名、性别、老师类型、年级 ===
      ctx.fillStyle = '#1a202c'
      ctx.font = 'bold 32px "Microsoft YaHei", "PingFang SC", Arial, sans-serif'
      ctx.textAlign = 'left'
      const nameText = teacher.value.name || '未填写'
      ctx.fillText(nameText, infoX, infoY)
      
      // 计算姓名宽度，用于标签定位
      const nameWidth = ctx.measureText(nameText).width
      
      // 第一排信息（性别、教师类型-年级）- 使用普通文字，不用标签
      const firstRowItems = []
      if (teacher.value.gender) firstRowItems.push(teacher.value.gender)
      if (teacher.value.teacher_type) {
        // 只显示教师类型和年级，不显示学历
        const typeLabel = getTeacherTypeLabel(teacher.value.teacher_type, teacher.value.grade_level, null)
        firstRowItems.push(typeLabel)
      }
      
      if (firstRowItems.length > 0) {
        ctx.font = '22px "Microsoft YaHei", "PingFang SC", Arial, sans-serif'
        ctx.fillStyle = '#2d3748' // 黑色文字
        ctx.textAlign = 'left'
        // 信息起始位置：姓名后面留出间距
        const infoText = firstRowItems.join('  ')
        ctx.fillText(infoText, infoX + nameWidth + 25, infoY)
      }
      ...
      const age = calculateAge(teacher.value.birthday)
      if (age) secondRowItems.push(`${age}岁`)
      if (teacher.value.teaching_years) secondRowItems.push(`${teacher.value.teaching_years}年教龄`)
      if (teacher.value.hometown) secondRowItems.push(`籍贯：${teacher.value.hometown}`)
      ...
      if (secondRowItems.length > 0) {
        ...
        secondRowItems.forEach((item) => {
          const itemWidth = ctx.measureText(item).width + 26
          
          // 标签背景 - 淡绿色
          ctx.fillStyle = '#E8F8F2'
          roundRect(ctx, itemX, infoY - 22, itemWidth, 34, 17)
          ctx.fill()
          
          // 标签文字
          ctx.fillStyle = '#3BA888'
          ctx.textAlign = 'center'
          ctx.fillText(item, itemX + itemWidth / 2, infoY - 1)
      ...
      // 更新 y 坐标（取头像底部和信息底部的较大值）
      y = Math.max(y + 120, infoY + 15)
      ...
      if (teacher.value.self_intro && teacher.value.self_intro.trim()) {
        ...
        y = wrapText(ctx, teacher.value.self_intro, padding + 15, y, contentWidth - 15, 45, 5) // 最多5行
        y += 20 // 板块底部间距
      }
      
      // === 个人优势 ===
      // 只有当个人优势有内容或有优势标签时才显示此板块
      if ((teacher.value.personal_advantage && teacher.value.personal_advantage.trim()) || 
          (teacher.value.advantage_tags && teacher.value.advantage_tags.length > 0)) {
        ...
        if (teacher.value.personal_advantage && teacher.value.personal_advantage.trim()) {
          ctx.font = '21px "Microsoft YaHei", "PingFang SC", Arial, sans-serif'
          ctx.fillStyle = '#4a5568'
          y = wrapText(ctx, teacher.value.personal_advantage, padding + 15, y, contentWidth - 15, 45, 4) // 最多4行
          y += 30 // 优势文字和标签之间的间距
        }
        
        // 显示优势标签
        if (teacher.value.advantage_tags && teacher.value.advantage_tags.length > 0) {
          const tags = teacher.value.advantage_tags.slice(0, 6) // 最多显示6个标签
          let tagX = padding + 15
          const tagY = y
          const lineHeight = 45
          ...
          tags.forEach((tag, index) => {
            ...
            // 检查是否需要换行
            if (currentLineWidth + tagWidth > maxLineWidth && currentLineWidth > 0) {
              y += lineHeight
              tagX = padding + 15
              currentLineWidth = 0
            }
            ...
            // 绘制标签背景
            ctx.fillStyle = '#FEF3C7'
            roundRect(ctx, tagX, y - 24, tagWidth, 36, 18)
            ctx.fill()
            
            // 绘制标签文字
            ctx.fillStyle = '#92400E'
            ctx.textAlign = 'center'
            ctx.fillText(tagText, tagX + tagWidth / 2, y)
            
            tagX += tagWidth + 12
            currentLineWidth += tagWidth + 12
          })
          
          ctx.textAlign = 'left'
          y += lineHeight + 20 // 标签后的间距
        } else {
          y += 20 // 如果没有标签
        }
      }
      ...
      if (teacher.value.experience && teacher.value.experience.length > 0) {
        ...
        experiencesToShow.forEach((exp, index) => {
          ...
          ctx.textAlign = 'left'
          const title = `${index + 1}. ${exp.subject || '教学经历'}`
          ctx.fillText(title, padding + 15, y)
          y += 45
          ...
          if (exp.start_date || exp.end_date) {
            const timeText = `${exp.start_date || ''} ${exp.start_date && exp.end_date ? '-' : ''} ${exp.end_date || ''}`
            if (timeText.trim()) {
              ctx.fillText(timeText.trim(), padding + 30, y)
              y += 38
            }
          }
          
          if (exp.location) {
            ctx.fillText(`地点：${exp.location}`, padding + 30, y)
            y += 38
          }
          ...
          if (exp.description) {
            ctx.fillStyle = '#4a5568'
            ctx.font = '20px "Microsoft YaHei", "PingFang SC", Arial, sans-serif'
            y = wrapText(ctx, exp.description, padding + 30, y, contentWidth - 45, 38, 3) // 最多3行
          } else {
            // 如果没有描述，减少一些间距
            y -= 10
          }
        ...
        y += 10 // 板块底部间距（从20减少到10）
      }
      
      // 绘制底部文字
      const footerY = finalHeight - 40
...
const submitCertificationReview = async () => {
  try {
    ...
    const res = await fetch(`/api/admin/teachers/${teacher.value.id}/review`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
      credentials: 'include',
      body: JSON.stringify({
        status: certificationForm.value.review_status,
        reason: certificationForm.value.review_note,
        real_name_verified: certificationForm.value.real_name_verified,
        education_verified: certificationForm.value.education_verified,
        teacher_verified: certificationForm.value.teacher_verified
      })
    })
    ...
    if (data.success) {
      ...
      ElMessage.error(data.error || '审核提交失败')
    }
  } catch (error) {
