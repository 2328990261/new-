...
class RecognitionService
{
    ...
    private $salaryPatterns = [
        ...
        // 🔥 新增：200/一小时、200/2小时、200元一小时、200元2小时 格式
        '/(\d+(?:[-.．]\d+)?)\s*(?:[-~到至]\s*(\d+(?:[-.．]\d+)?))?\s*(?:元)?[\/每]([一二三四五六七八九十\d\.]+)\s*(?:个)?(?:小时|h|H)/',
        // 🔥 新增：200/2h、200元/2h 格式
        '/(\d+(?:[-.．]\d+)?)\s*(?:[-~到至]\s*(\d+(?:[-.．]\d+)?))?\s*(?:元)?[\/](\d+(?:\.\d+)?)\s*(?:h|H)/',
        // 🔥 新增：200元n小时、200/n次 格式（n为数字）
        '/(\d+(?:[-.．]\d+)?)\s*(?:[-~到至]\s*(\d+(?:[-.．]\d+)?))?\s*(?:元)?[\/](\d+(?:\.\d+)?)\s*(?:小时|次|节|课时)/',
        // 🔥 新增：n小时200元 格式（时长在前）
        '/([一二三四五六七八九十\d\.]+)\s*(?:个)?(?:小时|h|H)[：:\s]*(\d+(?:[-.．]\d+)?)\s*(?:[-~到至]\s*(\d+(?:[-.．]\d+)?))?\s*(元|块|块钱)?/',
        // 标准格式：200元/小时、200/次
    ...
    public function batchRecognize($text)
    {
        ...
        // 🔥 关键修复：将只包含空白字符的行转换为真正的空行
        // 匹配：换行符 + 只有空白字符（空格、制表符等）的行 + 换行符
        // 替换为：两个换行符（真正的空行）
        $text = preg_replace('/\n[ \t]*\n/', "\n\n", $text);
    ...
    public function recognizeSingle($text)
    {
        ...
        $result = [
            ...
            'subject_id'    => null,
            'subject_name'  => null,
            'subject_ids'   => null,      // 多科目ID数组
            'subject_names' => null,      // 多科目名称数组
            'salary'        => null,
            'teacher_type'  => null,
            'teacher_gender' => null,
        ];
        ...
        if ($subject) {
            ...
            // 如果识别到多个科目，添加额外字段
            if (!empty($subject['is_multi'])) {
                $result['subject_ids'] = $subject['subject_ids'];
                $result['subject_names'] = $subject['subject_names'];
            }
        }
     ...
     * 识别科目（支持多科目）
     * @param string $text 文本
     * @return array|null 科目信息（单科目时返回单个，多科目时返回第一个，同时设置 subject_ids 和 subject_names）
     */
    public function recognizeSubject($text)
    {
        ...
        foreach ($subjects as $subject) {
            if (strpos($text, $subject->name) !== false) {
                $matchedSubjects[] = $subject->toArray();
            }
        }
        ...
        // 返回第一个匹配的科目（兼容原有逻辑）
        // 同时在结果中包含所有匹配的科目信息
        $result = $matchedSubjects[0];
        
        // 如果有多个科目，添加额外字段
        if (count($matchedSubjects) > 1) {
            $result['subject_ids'] = array_column($matchedSubjects, 'id');
            $result['subject_names'] = array_column($matchedSubjects, 'name');
    ...
    {
        // 🔥 新增：先尝试匹配带时长的特殊格式
        // 格式：200/一小时、200元2小时、200/2h
        if (preg_match('/(\d+(?:[-.．]\d+)?)\s*(?:[-~到至]\s*(\d+(?:[-.．]\d+)?))?\s*(?:元)?[\/每]([一二三四五六七八九十\d\.]+)\s*(?:个)?(?:小时|h|H)/u', $text, $matches)) {
            $amount1 = $matches[1];
            $amount2 = $matches[2] ?? '';
            $hours = $this->convertChineseNumber($matches[3]);
            
            // 格式化输出
            if ($amount2 && $amount1 != $amount2) {
                return $amount1 . '-' . $amount2 . '/' . $hours . '小时';
            } else {
                return $amount1 . '/' . $hours . '小时';
            }
        }
        
        // 🔥 新增：n小时200元 格式（时长在前）
        if (preg_match('/([一二三四五六七八九十\d\.]+)\s*(?:个)?(?:小时|h|H)[：:\s]*(\d+(?:[-.．]\d+)?)\s*(?:[-~到至]\s*(\d+(?:[-.．]\d+)?))?\s*(?:元|块|块钱)?/u', $text, $matches)) {
            $hours = $this->convertChineseNumber($matches[1]);
            $amount1 = $matches[2];
            $amount2 = $matches[3] ?? '';
        ...
        // 🔥 新增：200/2h 格式
        if (preg_match('/(\d+(?:[-.．]\d+)?)\s*(?:[-~到至]\s*(\d+(?:[-.．]\d+)?))?\s*(?:元)?[\/](\d+(?:\.\d+)?)\s*(?:h|H)/', $text, $matches)) {
            $amount1 = $matches[1];
            $amount2 = $matches[2] ?? '';
            $hours = $matches[3];
            
            if ($amount2 && $amount1 != $amount2) {
                return $amount1 . '-' . $amount2 . '/' . $hours . 'h';
            } else {
                return $amount1 . '/' . $hours . 'h';
            }
        }
     ...
     * @return string 阿拉伯数字字符串
     */
    private function convertChineseNumber($str)
    {
        $chineseNumbers = [
            '一' => '1', '二' => '2', '三' => '3', '四' => '4', '五' => '5',
        ...
        // 处理简单的中文数字
        if (isset($chineseNumbers[$str])) {
            return $chineseNumbers[$str];
        }
        
        // 处理复合中文数字（如：一点五、二点五）
        if (preg_match('/([一二三四五六七八九])点([一二三四五六七八九五])/', $str, $matches)) {
            $integer = $chineseNumbers[$matches[1]] ?? $matches[1];
            $decimal = $chineseNumbers[$matches[2]] ?? $matches[2];
